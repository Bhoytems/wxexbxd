...
class TradingBot:
    ...
    def connect(self, email, password, account_type='PRACTICE'):
        try:
            ...
            print(f"IQ Option connect: check={check}, reason={reason}")
            ...
            if check:
                ...
                if "2fa" in reason_str or "two" in reason_str:
                    return False, "IQ Option requires 2FA verification"
                elif "email" in reason_str or "confirm" in reason_str or "verif" in reason_str:
                    return False, "EMAIL_CONFIRMATION: Check your email and click the verification link IQ Option sent, then try again"
                elif "invalid" in reason_str or "wrong" in reason_str or "password" in reason_str:
                    return False, "Wrong email or password. Please check and try again"
                elif "block" in reason_str:
                    return False, "Your account has been blocked by IQ Option"
                else:
                    return False, f"IQ Option rejected login: {reason}"
