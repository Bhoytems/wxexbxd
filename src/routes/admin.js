...
router.post('/redeem-codes', async (req, res) => {
  const { code, reward, maxUses, expiresInHours } = req.body;
  if (!code || !reward) {
    return res.status(400).json({ error: 'code and reward are required' });
  }

  // Expiry is optional. If provided, it must be a positive number of hours, capped at 48.
  let expiry = null;
  if (expiresInHours !== undefined && expiresInHours !== null && expiresInHours !== '') {
    expiry = parseFloat(expiresInHours);
    if (isNaN(expiry) || expiry <= 0 || expiry > 48) {
      return res.status(400).json({ error: 'expiresInHours must be a number between 0 and 48' });
    }
  }
