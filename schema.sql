...
CREATE TABLE IF NOT EXISTS redeem_codes (
  ...
  active BOOLEAN DEFAULT TRUE,
  expires_at TIMESTAMP,       -- NULL = never expires; otherwise capped at 48h from creation (enforced in admin route)
  created_at TIMESTAMP DEFAULT NOW()
);

-- Safe to run against an already-existing database (adds the column if it's missing)
ALTER TABLE redeem_codes ADD COLUMN IF NOT EXISTS expires_at TIMESTAMP;
